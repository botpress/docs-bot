...
const TITLE_CACHE_KEY = 'marg-conv-titles'
...
export function Header({
  ...
  const displayTitle = (activeTitle && stripMarkdown(stripPagePrefix(activeTitle))) || 'New chat'

  return (
    <header
      ref={wrapperRef}
      className="relative px-2 py-2.5 flex items-center justify-between gap-2 border-b border-border/40"
    >
      <div className="flex items-center gap-1 min-w-0">
        <IconButton onClick={() => postParent('closePanel')} label="Collapse panel">
          <PanelRightClose className="size-4" />
        </IconButton>
        <span className="text-[13px] font-medium text-foreground truncate">{displayTitle}</span>
      </div>
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        <IconButton
          onClick={() => {
            setExpanded((v) => !v)
            postParent('toggleWidth')
          }}
          label={expanded ? 'Restore width' : 'Expand panel'}
        >
          {expanded ? <Minimize2 className="size-4" /> : <Maximize2 className="size-4" />}
        </IconButton>
        <IconButton onClick={handleNew} label="New conversation">
...
function IconButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      {...(ariaExpanded !== undefined ? { 'aria-haspopup': 'menu' as const, 'aria-expanded': ariaExpanded } : {})}
...
function ConversationMenu({
  ...
  return (
    <div
      role="menu"
      className={cn(
        'absolute z-20 right-2 top-[calc(100%-4px)] w-[280px]',
...
function previewOf(c: Conversation, cachedTitle?: string): { title: string; subtitle?: string } {
  ...
  if (cachedTitle) {
    const t = stripMarkdown(stripPagePrefix(cachedTitle))
    if (t) return { title: t, subtitle: time }
  }
  ...
  text = text ? stripMarkdown(stripPagePrefix(text)) : undefined
  if (text) return { title: text, subtitle: time }
  return { title: 'New conversation', subtitle: time }
}

/** Drop the hidden `__page:<path>|` prefix from page-tagged user messages. */
function stripPagePrefix(text: string): string {
  if (!text.startsWith('__page:')) return text
  const bar = text.indexOf('|')
  return bar >= 0 ? text.slice(bar + 1).trim() : ''
}
