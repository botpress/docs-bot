    ...
    const handleMessage = (event: MessageEvent) => {
      if (event.data.type === "askAI" && event.data.data?.path) {
        setCurrentContext((prev) => {
          if (!prev.some((item) => item.path === event.data.data.path)) {
            return [
              ...prev,
              { title: event.data.data.title, path: event.data.data.path },
            ];
          }
          return prev;
        });
      ...
      if (
        ...
        if (!isLandingPage && currentContext.length === 0 && isNewPath) {
          const alreadyInContext = currentContext.some(
            (item) => item.path === event.data.data.path
          );
          if (!alreadyInContext) {
            setSuggestedContext({
